...
namespace TVMEditor.Test.Experiments
{
    public class Levi
    {
        public static void Run(string inputDir, string outputDir)
        {
            ...
            var (indices, transformations) = TransformationsIO.LoadIndexedTransformations($"{inputDir}/indices.txt", $"{inputDir}/transformations.txt");
            ...
            MeshIO.WriteSequenceToObj($"{outputDir}/Levi/dq", deformedSequence);
        }
    }
}

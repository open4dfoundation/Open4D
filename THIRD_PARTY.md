...
| `open4d/reconstruction/streamer` | Project-authored Python and browser client, written in-repo. The only third-party material is `streamer/client/vendor/draco/`: `draco_wasm_wrapper.js` (SHA-256 `e8049906ef3f8f75d3456c22a3f31bfdfe5b5b5bd09ccdec613b9e9a49d554d8`) and `draco_decoder.wasm` (SHA-256 `2516a4e43526d71787bf2f678f951329f7f858f8f15f42d4bc9e370b31a0da3a`), copied from `open4d/codecs/tsmc/draco/javascript/` and byte-identical to the pinned `google/draco` submodule `47238930f698250f474163e1a29d77858aa5c158`. The client loads no CDN or other external origin | Root MIT is current repository evidence for the project-authored source; `pyproject.toml` declares no license. Vendored Draco is Apache-2.0, its license text held in the codec submodule tree rather than beside the copy | `EXCLUDED`; declare the component license, place the Apache-2.0 notice and text beside the vendored decoder, and re-verify both hashes whenever the matched wrapper/module pair is refreshed |

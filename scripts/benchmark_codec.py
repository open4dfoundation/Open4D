...
def run(
    ...
    selected = _BASELINES.get(codec, codec)
    info = _BASELINES.get(codec) or next(item for item in available_codecs() if item.id == codec)
    encode = lambda: encode_sequence(
        source, output, codec=selected, overwrite=True, **(encode_options or {})
    )
    artifact, encode_s = timed(encode)
    encode_peak = peak_bytes(encode)
    decoded, open_s = timed(
        lambda: decode_sequence(artifact, codec=selected, **(decode_options or {}))
    )
    try:
    ...
    return {
        ...
        "vertices": vertices,
        "triangles": triangles,
        "artifact_bytes": (sum(item.stat().st_size for item in artifact.rglob("*") if item.is_file())
...
def main() -> None:
    ...
    imported = (open_sequence(args.source, fps=30)
                if args.source else synthetic(args.side, args.frames))
    with imported:
        lazy = imported[:args.frames]
    ...
    suffixes = {info.id: info.suffixes[0] for info in (*available_codecs(), *_BASELINES.values())}

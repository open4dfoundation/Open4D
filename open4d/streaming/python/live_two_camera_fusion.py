...
class FrameServer:
        ...
        for item in frame.payloads:
            ...
            if item.stream_type == protocol.STREAM_COLOR:
                ...
                if (
                    ...
                    or item.format != protocol.FORMAT_DEPTH16_LE
                    or item.width != WIDTH
                    or item.height != HEIGHT
                    or item.raw_length != DEPTH_BYTES
                ):
                    raise protocol.ProtocolError("invalid depth payload")
                content_size = zstandard.frame_content_size(item.data)
                if content_size not in (zstandard.CONTENTSIZE_UNKNOWN, DEPTH_BYTES):
                    raise protocol.ProtocolError("invalid compressed depth size")
                raw = self.decompressor.decompress(
                    item.data, max_output_size=DEPTH_BYTES, allow_extra_data=False
                )
    ...
    def close(self) -> None:
        self.stop.set()
        sockets = (self._connection, self._listener)
        for sock in sockets:
            if sock is not None:
                try:

...
def build_frame(pair_dir: Path, delay_usec: int) -> protocol.Frame:
    ...
    for entry in metadata["payloads"]:
        path = (pair_dir / entry["file"]).resolve()
        if not path.is_relative_to(pair_dir):
            raise ValueError(f"payload path leaves the capture pair: {entry['file']}")
        with path.open("rb") as source:
            data = source.read(protocol.MAX_SINGLE_PAYLOAD + 1)
        if len(data) > protocol.MAX_SINGLE_PAYLOAD:
            raise protocol.ProtocolError("capture payload exceeds maximum size")

...
void Reconstruction::run()
{
  ...
  {
    extrinsic_tf_list[i] = calib.at(i).o3d_extrinsic;
    intrinsic_list[i] = calib.at(i).o3d_intrinsic;
  }

  t::geometry::TriangleMesh mesh;
  std::vector<ImageFrame> cam_frame(this->device_count_);
  ...
  try
  {
    while (1)
    {
      ...
      for (int i = 0; i < this->device_count_; i++)
      {
        ...
        if (cam_frame.at(i).o3d_depImg.IsEmpty())
          throw std::runtime_error("reconstruction received an empty depth image");
        t::geometry::Image depth_img = cam_frame.at(i).o3d_depImg.To(GPU_DEVICE);

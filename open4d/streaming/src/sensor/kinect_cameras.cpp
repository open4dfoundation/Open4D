...
static open3d::t::geometry::Image copy_image(
    const cv::Mat& image, open3d::core::Dtype dtype)
{
  open3d::core::Tensor tensor(
      {image.rows, image.cols, image.channels()}, dtype, CPU_DEVICE);
  const size_t row_bytes = image.cols * image.elemSize();
  auto* destination = static_cast<uint8_t*>(tensor.GetDataPtr());
  for (int row = 0; row < image.rows; ++row)
  {
    std::memcpy(destination + row * row_bytes, image.ptr(row), row_bytes);
  }
...
void KinectCameras::run(uint32_t idx, std::string* serial_num)
{
  ...
  *serial_num = cap->serial_num;
  ...
  while (!stop_cameras)
  {
    ...
    cv::Mat color;
    cv::cvtColor(cap->cv_color_img_rectified, color, cv::COLOR_BGRA2RGB);
    cv::Mat depth = cap->cv_depth_img_rectified.clone();
    auto o3d_color = copy_image(color, open3d::core::Dtype::UInt8);
    open3d::t::geometry::Image o3d_depth;
    if (this->enable_depth_)
    {
      if (depth.empty() || depth.type() != CV_16UC1)
        throw std::runtime_error("capture did not produce a uint16 depth image");
      o3d_depth = copy_image(depth, open3d::core::Dtype::UInt16);
    }

    this->out_->put(
        idx,
        {.cv_colImg = std::move(color),
         .cv_depImg = std::move(depth),

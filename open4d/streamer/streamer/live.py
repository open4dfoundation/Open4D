...
def attach(bundle_dir, clip, *, replace: bool = False) -> Path:
    """Add a live clip to an existing bundle. See `bundle.add`.

    Kept as a name here because starting a renderer and pointing a bundle at it
    is one action from a producer's point of view, and this is where the other
    half of it lives.
    """
    return bundle.add(bundle_dir, clip, replace=replace)

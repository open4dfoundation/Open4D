...
namespace sibr {
	...
	) : filepath(_filepath), fps(_fps), _forceResize(forceResize)
	{
		...
		/** Init FFMPEG, registering available codec plugins.
		    av_register_all() was deprecated in FFmpeg 4.0 and removed in 5.0;
		    registration is automatic from 4.0 onwards, so this is now a no-op. */
		if (!ffmpegInitDone) {
	...
	void FFVideoEncoder::close()
	{
		...
		if (video_st) {
			// avcodec_close() on a stream-owned context is gone in FFmpeg 5.0;
			// the separately allocated context is freed instead.
	...
	void FFVideoEncoder::init(const sibr::Vector2i & size)
	{
		...
		/** AVStream::codec was removed in FFmpeg 5.0. The encoder context is now
		    allocated separately and copied into the stream's codecpar below. */
		...
		// The stream carries the parameters, not the context, since FFmpeg 5.0.
		if (avcodec_parameters_from_context(video_st->codecpar, pCodecCtx) < 0) {
			SIBR_WRG << "[FFMPEG] Failed to copy codec parameters." << std::endl;
			return;
		}
	...
	{
		/** avcodec_encode_video2() was removed in FFmpeg 5.0. One frame in may
		    now yield zero or more packets out, so the receive side is a loop.
		    EAGAIN means "no packet yet", which is normal, not an error. */

...
def _to_vertex_tensor(vertices, device):
    if isinstance(vertices, torch.Tensor):
        return vertices.to(device=device, dtype=torch.float32)
    return torch.as_tensor(vertices, dtype=torch.float32, device=device)

def _to_face_tensor(faces, device):
    if isinstance(faces, torch.Tensor):
        return faces.to(device=device, dtype=torch.int32)
    return torch.as_tensor(faces, dtype=torch.int32, device=device)

def render_mesh(mesh, mv, mvp, iter_res, return_types = ["mask", "depth"], white_bg=False):

    #print(mesh.vertices.dtype)
    #print(torch.tensor(mesh.vertices, dtype=torch.float32, device=mvp.device))
    #print(torch.tensor(mesh.vertices, dtype=torch.float32, device=mvp.device).dtype)

    v_pos = _to_vertex_tensor(mesh.vertices, mvp.device)
    v_pos_clip = xfm_points(v_pos.unsqueeze(0), mvp)  # Rotate it to camera coordinates
    tri = _to_face_tensor(mesh.faces, mvp.device)

    rast, db = dr.rasterize(
        #dr.RasterizeGLContext(), v_pos_clip, mesh.faces.int(), iter_res)
        dr.RasterizeGLContext(), v_pos_clip, tri, iter_res)

    out_dict = {}
    for type in return_types:
        if type == "mask" :
            img = dr.antialias((rast[..., -1:] > 0).float(), rast, v_pos_clip, tri) 
        elif type == "depth":
            v_pos_cam = xfm_points(v_pos.unsqueeze(0), mv)
            img, _ = interpolate(v_pos_cam, rast, tri)
        elif type == "normal" :
            nrm = _to_vertex_tensor(mesh.nrm, mvp.device)
            normal_indices = (torch.arange(0, nrm.shape[0], dtype=torch.int64, device=mvp.device)[:, None]).repeat(1, 3)
            img, _ = interpolate(nrm.unsqueeze(0).contiguous(), rast, normal_indices.int())
        if white_bg:

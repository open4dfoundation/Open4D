...
for FILE in ${cfg_all[@]} cfg-site-default.yaml cfg-tools-trackedmode.yaml sequences-trackedmode.yaml cfg-site.yaml
do
...
do_one_cfgset() {
	...
	CMD="${CURDIR}/gen-cfg.pl \
		...
		${CFGDIR}cfg-tools-trackedmode.yaml \
		${CONDCFG[@]} \
		${CFGDIR}sequences-trackedmode.yaml \

...
{
    public class TransformationMatrixPentExperiment1
    {
        public static void Run(string inputDir, string outputDir)
        {
            ...
            var (indices, transformations) = TransformationsIO.LoadIndexedTransformations($"{inputDir}/indices_1.txt", $"{inputDir}/transformations_1.txt");

            var affinityCalculation = new DistanceDirectionAffinityCalculation
            {
                ShapeDistance = 1f
            };
            var centersDeformations = new AffinityCenterDeformationMatrix(affinityCalculation);
            ...
            MeshIO.WriteSequenceToObj($"{outputDir}/pent/matrix_1", deformedSequence);
        }
    }
}

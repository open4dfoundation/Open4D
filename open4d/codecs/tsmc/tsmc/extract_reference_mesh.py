...
parser.add_argument('--key', type=int, default=5, help="key mesh")
...
for i in range(firstIndex, lastIndex + 1):
    ...
    all_meshes += mesh
    #print(i, deformed_meshes[i-firstIndex])
...
GoF = 0
fitting_meshes = []
for i in range(key-1, key+1):
    GoF += 1
    if i == key:
        continue
    deformed_number_of_triangles = round(np.array(deformed_meshes[key].triangles).__len__() / 4)
    decimated_mesh_i = o3d.geometry.TriangleMesh.simplify_quadric_decimation(deformed_meshes[i], deformed_number_of_triangles, boundary_weight=8000)
...
if not os.path.exists(os.path.join(outputDir, "others")):
    os.makedirs(os.path.join(outputDir, "others"))
o3d.io.write_triangle_mesh(os.path.join(outputDir, "others", "undecimated_reference_mesh.obj"), pre_reference_mesh, write_vertex_normals=False, write_vertex_colors=False, write_triangle_uvs=False)
...
print("Reference mesh saved!")
print("Find reference mesh here: ", outputDir)

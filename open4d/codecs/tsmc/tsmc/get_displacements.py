...
for obj_file in selected_obj_files:
    dynamic_deformed = o3d.io.read_triangle_mesh(f'{EDITOR_BUILD}/output/{dataset}_{num_centers}/reference/deformed_reference_mesh_{i:03}.obj')
    ...
    o3d.io.write_triangle_mesh(f'{EDITOR_BUILD}/output/{dataset}_{num_centers}/reference/fitting_mesh_{i:03}.obj', fitting_mesh_dancer_i, write_vertex_normals=False, write_vertex_colors=False, write_triangle_uvs=False)
    #o3d.visualization.draw_geometries([fitting_mesh_dancer_i])
    i += 1

loaded_decimated_reference_mesh = o3d.io.read_triangle_mesh(f'{EDITOR_BUILD}/Data/{dataset}_{num_centers}/reference_mesh/decimated_reference_mesh.obj', enable_post_processing=False)
print(loaded_decimated_reference_mesh)
result = subprocess.run([
    '../draco/build/draco_encoder',
    '-i', f'{EDITOR_BUILD}/Data/{dataset}_{num_centers}/reference_mesh/decimated_reference_mesh.obj',
    '-o', f'{EDITOR_BUILD}/Data/{dataset}_{num_centers}/reference_mesh/decimated_reference_mesh.drc',
    '-qp', str('14'),
    '-cl', '7'
...
result = subprocess.run([
    ...
    '-i', f'{EDITOR_BUILD}/Data/{dataset}_{num_centers}/reference_mesh/decimated_reference_mesh.drc',
    '-o', fr'{EDITOR_BUILD}/Data/{dataset}_{num_centers}/reference_mesh/others/decoded_decimated_reference_mesh.obj'
], capture_output=True, text=True)
print(result.stdout)
print(result.stderr)

loaded_decoded_decimated_reference_mesh = o3d.io.read_triangle_mesh(f'{EDITOR_BUILD}/Data/{dataset}_{num_centers}/reference_mesh/others/decoded_decimated_reference_mesh.obj', enable_post_processing=False)
...
for i in range(firstIndex, lastIndex + 1):
    fitting_mesh_dancer_i = read_triangle_mesh_with_trimesh(f'{EDITOR_BUILD}/output/{dataset}_{num_centers}/reference/fitting_mesh_{i:03}.obj', enable_post_processing=False)
    ...
    np.savetxt(f'{EDITOR_BUILD}/output/{dataset}_{num_centers}/reference/displacements_{dataset}_{i:03}.txt', displacement_i, fmt='%8f')
...
for i in range(firstIndex, lastIndex + 1):
    displacement_path = f'{EDITOR_BUILD}/output/{dataset}_{num_centers}/reference/displacements_{dataset}_{i:03}.txt'
...
for k in range(firstIndex, lastIndex+1):
    ...
    reconstructed_displacement = np.loadtxt(f'{EDITOR_BUILD}/output/{dataset}_{num_centers}/reference/displacements_{dataset}_{k:03}.txt')
...
for i in range(firstIndex, lastIndex + 1):
    ...
    o3d.t.io.write_point_cloud(f'{EDITOR_BUILD}/Data/{dataset}_{num_centers}/reference_mesh/dis_{dataset}_{i:03}.ply', pc, write_ascii=True)

times = []
input_encoder_path = fr"{EDITOR_BUILD}/Data/{dataset}_{num_centers}/reference_mesh"
output_encoder_path = fr"{EDITOR_BUILD}/Data/{dataset}_{num_centers}/reference_mesh/GoF{num_frames}"
...
for i in range(firstIndex, lastIndex + 1):
    result = subprocess.run([
        '../draco/build/draco_decoder',
        '-i',
        fr'{EDITOR_BUILD}/Data/{dataset}_{num_centers}/reference_mesh/GoF{num_frames}/dis_{dataset}_{i:03}.drc',
        '-o',
        fr'{EDITOR_BUILD}/Data/{dataset}_{num_centers}/reference_mesh/GoF{num_frames}/decoded_{dataset}_{i:03}_displacements.ply'
    ], capture_output=True, text=True)
    print(result.stdout)

...
namespace TVMEditor.Test
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (!(mode=="1" || mode=="2"))
            {
                Console.WriteLine("Error: <mode> must be 1 or 2, mode 1 for getting reference mesh, mode 2 for deforming reference mesh to each frame in the group.");
                return;
            }
            ...
            center_affinity_deformation.Run(dataset, inputDir, outputDir, firstIndex, lastIndex, mode);
            /*
            switch (dataset)
            {
                case "dancer":

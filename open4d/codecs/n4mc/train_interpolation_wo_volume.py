...
def train_interpolation(args):
    ...
    for epoch in range(1, args.n_epoch + 1):
        for step, data_dict in enumerate(data_loader):
            ...
            # Forward pass
            pred_f_intermediate = net(f_start, f_end, batch_latent_codes)  # (batch, group_size-2, 4, 4, 4, 16)
        ...
        if epoch % args.val_frequence == 0:
            ...
            for index_t, data_dict in enumerate(tqdm(val_data_loader, desc=f'val epoch {epoch}')):
                try:
                    ...
                    latent_code_list.append(batch_latent_codes.detach().cpu())
                    f_start, f_end = embed_features[:, 0], embed_features[:, -1]
                    start = time.time()
                    # Forward pass
                    pred_f_intermediate = net(f_start, f_end, batch_latent_codes)  # (1, group_size-2, 4, 4, 4, 16)
                    #print("pred_f_intermediate: ", pred_f_intermediate.shape)
                    ...
                    pass
            print('decoding time network: ', np.mean(decoding_time_network))
